...
public class CreateUserBean implements CommandLineRunner {
    ...
    private void createRoles() {
        if (roleRepository.findByName("product-manager").isPresent()) return;
    ...
    private void createCategories() {
        if (categoryRepository.existsBySlug("elektronik")) return;
    ...
    private void createProducts() {
        ...
        if (productVariantRepository.findBySku("IP15PRO-SYH-256").isPresent()) return;

        List<CategoryEntity> cats = categoryRepository.findAll();
        CategoryEntity telefon    = cats.stream().filter(c -> c.getSlug().equals("telefon-aksesuar")).findFirst().orElse(cats.get(0));
        CategoryEntity bilgisayar = cats.stream().filter(c -> c.getSlug().equals("bilgisayar-tablet")).findFirst().orElse(cats.get(0));
        CategoryEntity erkek      = cats.stream().filter(c -> c.getSlug().equals("erkek-giyim")).findFirst().orElse(cats.get(0));
        CategoryEntity spor       = cats.stream().filter(c -> c.getSlug().equals("spor-outdoor")).findFirst().orElse(cats.get(0));
    ...
    private void createCarts() {
        ...
        if (v1 != null) addToCart("customer1@example.com", v1, 1);
        if (v2 != null) addToCart("customer1@example.com", v2, 2);
        if (v3 != null) addToCart("customer3@example.com", v3, 1);
    }
    ...
    private void addToCart(String email, ProductVariantEntity variant, int qty) {
        userRepository.findByEmailAndDeletedAtIsNull(email).ifPresent(user -> {
            CartEntity cart = cartRepository.findByUserId(user.getId()).orElseGet(() ->
                    cartRepository.save(CartEntity.builder().user(user).build()));
    ...
    private void createOrders() {
        ...
        Map<String, Object> addr1 = shippingAddr("Ali Yılmaz", "Bağdat Cad. No:42 D:8", istanbul.getName(), "34710");
        Map<String, Object> addr2 = shippingAddr("Ayşe Kaya",  "Barbaros Bulvarı No:15", istanbul.getName(), "34353");
        Map<String, Object> addr3 = shippingAddr("Mehmet Demir","Çamlıca Cad. No:7 D:2", istanbul.getName(), "34674");
        ...
        if (c2 != null && turkey != null) {
            ...
            refundRepository.save(RefundEntity.builder()
                    ...
                    .providerRefundId("MOCK-REF-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase())
